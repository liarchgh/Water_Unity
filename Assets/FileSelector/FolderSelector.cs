...
{	
	...
	{	
		if(open)
		{
			...
			GUI.Window(0, windowDimensions, DrawFileSelector, "选择文件夹");
		}
	}
	...
	public static void GetFile(string startingDirectory, SelectFileFunction Callback = null, string extension = ".*")
	{
		...
		FolderSelector instance = updater.AddComponent<FolderSelector>();
	...
	private void DrawFileSelector(int ID)
	{
		...
		if (GUILayout.Button ("选择文件夹")) {
			if (Callback != null)
				Callback (Status.Successful, path);
			open = false;
